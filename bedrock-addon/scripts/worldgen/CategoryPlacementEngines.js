...
function call(generator, names, args) {
  for (const name of names) if (typeof generator?.[name] === "function") return generator[name](...args);
  return null;
}
...
export class CategoryPlacementEngines {
  ...
  async water(candidate, context) {
    ...
    return call(this.generator, ["placeWaterStructure", "placeWater", "placeTemplate"], [candidateId(candidate), context.dimension, { ...context.location, y }, { ...context.options, waterlogged: true, waterLevel, seaFloorY: floorY, terrainAdaptation: adaptation }, context.placementKey]);
  }
  ...
  async underground(candidate, context) {
    ...
    return call(this.generator, ["placeUndergroundStructure", "placeUnderground", "placeTemplate"], [candidateId(candidate), context.dimension, { ...context.location, y }, { ...context.options, underground: true, terrainAdaptation: adaptation }, context.placementKey]);
  }
  ...
  async village(candidate, context) {
    ...
    return call(this.generator, ["placeGroundVillage", "placeVillage", "placeTemplate"], [candidateId(candidate), context.dimension, { ...context.location, y }, { ...context.options, village: true, terrainAdaptation: adaptation, flattenTerrain: true, foundationMode: adaptation.foundation }, context.placementKey]);
  }
