...
export class PlacementEngineOrchestrator {
  ...
  refresh(registry = this.generator?.structureRegistry ?? this.generator?.jigsawRegistry) { if (registry) this.registry = registry; this.adapters = createPlacementAdapters(this.generator); return this; }
  ...
  async place(candidate, context = {}) {
    ...
    } catch (error) { return { placed:false, reason:"placement_exception", error:String(error), classification }; }
  }
}
