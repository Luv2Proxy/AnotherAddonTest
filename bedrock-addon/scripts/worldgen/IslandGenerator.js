...
// Structure selection is intentionally broad. The original Java mod did not
// treat every vanilla structure as a single generic jigsaw family; this table
// keeps the same idea while allowing Bedrock's StructureManager/native jigsaw
// APIs to handle the actual assembly whenever possible.
...
export class IslandGenerator{
 ...
 chooseStructure(d){
   const nativeR=this.structureSeed(Math.floor(d.x),Math.floor(d.z),"native");
   let nativeCursor=0;
   for(const native of NATIVE_STRUCTURE_CHANCES){
     nativeCursor+=native.chance;
     if(nativeR<nativeCursor){
       const key=`native:${native.id}:${Math.floor(d.x/16)}:${Math.floor(d.z/16)}`;
       if(!this.placedStructureKeys.has(key))return{native:native.id,category:native.category,footprint:native.footprint};
       break;
     }
   }

   const r=this.structureSeed(Math.floor(d.x),Math.floor(d.z),"structure");
   let cursor=0;
   for(const cfg of ALL_STRUCTURE_CONFIG){
     cursor+=cfg.chance;
     if(r>=cursor)continue;
     const entry=this.registry.select(Math.floor(r*0x7fffffff),cfg.category,cfg.family);
     if(entry)return{entry,category:cfg.category,maxDepth:cfg.maxDepth,family:cfg.family,assetRole:assetRole(entry.id),assetKind:entry.kind};
   }
   return null;
 }
 ...
 placeQueuedStructures(){if(!this.structureJobs.length)return;const job=this.structureJobs.shift(),dim=world.getDimension(DIMENSION);try{let r;
   if(job.native==="stronghold"){const plan=this.native.plan("stronghold",{x:job.x,y:job.y,z:job.z,footprint:job.footprint,footprintRadius:48,searchRadius:32});r=this.native.commit("stronghold",plan);}
   else if(job.native==="mineshaft"){const plan=this.native.plan("mineshaft",{x:job.x,y:job.y,z:job.z,footprint:job.footprint,footprintRadius:32,searchRadius:24});r=this.native.commit("mineshaft",plan);}
   else if(job.native==="monument"){const plan=this.native.plan("jigsaw",{x:job.x,y:job.y,z:job.z,footprint:job.footprint,footprintRadius:28,searchRadius:32});r=this.native.commitOceanMonument({x:plan?.target?.x??job.x,y:plan?.target?.y??job.y,z:plan?.target?.z??job.z});}
   else if(job.assetKind==="jigsaw"){r=this.placement.placeJigsawStructure(job.id,dim,{x:job.x,y:job.y,z:job.z},{maxDepth:job.maxDepth,overlapPadding:2},job.category);}
   else if(job.assetKind==="composite"){r=this.placeCompositeStructure(job,dim);}
   else if(job.category===StructureCategory.UNDERGROUND)r=this.placement.placeTemplate(job.id,dim,{x:job.x,y:Math.max(-64,job.y-24),z:job.z},{rotation:"None"},job.category);
   else if(job.category===StructureCategory.WATER)r=this.placeWaterStructure(job,dim);
   else r=this.placement.placeTemplate(job.id,dim,{x:job.x,y:job.y,z:job.z},{rotation:"None"},job.category);
   if(r?.placed||r?.accepted){this.placedStructureKeys.add(job.key);this.save();}
 ...
 placeCompositeStructure(job,dim){
   // Prefer a real vanilla jigsaw root if the generated importer found one.
   // This is the most faithful path for Ancient City/Bastion/Trial Chambers.
   const root=this.registry.jigsawRoot(job.family);
   if(root?.definition){
     const nativeResult=this.placement.placeJigsawStructure(root.id,dim,{x:job.x,y:job.y,z:job.z},{maxDepth:job.maxDepth,overlapPadding:2},job.category);
     if(nativeResult?.placed)return nativeResult;
   }
   const pieces=this.registry.composite(job.family),roles=Object.keys(pieces);if(!roles.length)return{placed:false,reason:"empty_composite_mapping"};
   const role=roles[Math.floor(this.structureSeed(job.x,job.z,`role:${job.family}`)*roles.length)];
   const candidates=pieces[role];if(!candidates?.length)return{placed:false,reason:`empty_role:${role}`};
   const entry=candidates[Math.floor(this.structureSeed(job.x,job.z,`piece:${job.family}:${role}`)*candidates.length)];if(!entry)return{placed:false,reason:"missing_piece"};
   const offsets={entrance:[0,0,0],center:[0,0,0],structure:[8,0,8],wall:[-8,0,-8],bridge:[0,0,0],hoglin_stable:[8,0,8],treasure:[-8,0,8],units:[0,0,-8],room:[0,0,0]};
   const o=offsets[role]??[0,0,0];
   return this.placement.placeTemplate(entry.id,dim,{x:job.x+o[0],y:job.y+o[1],z:job.z+o[2]},{rotation:"None",overlapPadding:2},job.category);
 }
