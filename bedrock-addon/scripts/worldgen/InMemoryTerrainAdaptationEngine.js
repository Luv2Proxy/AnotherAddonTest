...
function solid(value) {
  const id = idOf(value);
  return !!id && id !== AIR && !WATER_IDS.has(id) && !LAVA_IDS.has(id);
}
function replaceable(value) {
  const id = idOf(value);
  return !id || id === AIR || WATER_IDS.has(id) || LAVA_IDS.has(id);
}
...
function set(map, x, y, z, id) {
  map.set(x, y, z, { id });
  map.updateSurfaceAfterSet(x, y, z);
}
function clear(map, x, y, z) {
  map.set(x, y, z, { id: AIR });
  map.updateSurfaceAfterSet(x, y, z);
}

/**
 * Terrain adaptation that never queries the live Dimension.
 * All operations are performed against TerrainVoxelMap and are intended to
 * run before the final bulk write.
 */
export class InMemoryTerrainAdaptationEngine {
    ...
    for (let x = box.minX; x <= box.maxX; x++) for (let z = box.minZ; z <= box.maxZ; z++) {
      ...
      if (current < target) {
        for (let y = current + 1; y <= target; y++) {
          const b = map.get(x, y, z);
          if (replaceable(b)) { set(map, x, y, z, foundation); this.stats.changed++; this.stats.foundations++; }
        }
        set(map, x, target, z, foundation);
  ...
  buryBox(map, box, job) {
    ...
    for (let x = box.minX; x <= box.maxX; x++) for (let z = box.minZ; z <= box.maxZ; z++) {
      const top = Math.floor(box.minY) - 1;
      const bottom = Math.max(map.minY, top - depth + 1);
  ...
  carveBox(map, box, job) {
    ...
    const minX = box.minX - padding, maxX = box.maxX + padding;
    const minY = box.minY, maxY = box.maxY;
    const minZ = box.minZ - padding, maxZ = box.maxZ + padding;
  ...
  fluidBox(map, box, fluid) {
    ...
    for (let x = box.minX; x <= box.maxX; x++) for (let y = box.minY; y <= box.maxY; y++) for (let z = box.minZ; z <= box.maxZ; z++) {
      if (replaceable(map.get(x, y, z))) { set(map, x, y, z, id); this.stats.changed++; }
    }
    this.stats.fluids++;
  }
  ...
  adaptJob(map, job) {
    ...
    if (mode === "none") return;
    const boxes = boundsForJob(job, this.options);
    for (const box of boxes) {
      ...
      else if (mode === "lav aline" || mode === "lav aline") this.fluidBox(map, box, "lava");
    }
    this.stats.jobs++;
  }
...
export function adaptTerrainMap(map, jobs, options = {}) {
  return new InMemoryTerrainAdaptationEngine(options).adaptCombined(map, jobs);
}
