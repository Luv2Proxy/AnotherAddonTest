...
const STRUCTURE_WEIGHT_RADIUS = 12;
const STRUCTURE_WEIGHT_SIZE = 24;
const STRUCTURE_WEIGHT_TABLE = new Float64Array(STRUCTURE_WEIGHT_SIZE ** 3);

// Vanilla's Beardifier precomputes a 24^3 beard kernel. The kernel is the
// Gaussian used by getBeardContribution; retaining it as a lookup table also
// matches vanilla's integer-offset lookup behavior.
...
// Vanilla's structure-weight table is the same style of precomputed radial
// falloff used by the structure-density path. Keep a separate table so the
// structure weighting path can be tuned independently from the beard kernel.
for (let x = 0; x < STRUCTURE_WEIGHT_SIZE; x++) {
  for (let y = 0; y < STRUCTURE_WEIGHT_SIZE; y++) {
    for (let z = 0; z < STRUCTURE_WEIGHT_SIZE; z++) {
      const dx = x - STRUCTURE_WEIGHT_RADIUS;
      const dy = y - STRUCTURE_WEIGHT_RADIUS;
      const dz = z - STRUCTURE_WEIGHT_RADIUS;
      STRUCTURE_WEIGHT_TABLE[(x * STRUCTURE_WEIGHT_SIZE + y) * STRUCTURE_WEIGHT_SIZE + z] = Math.exp(-(dx * dx + dz * dz) / 16.0 - (dy * dy) / 2.0);
    }
  }
}
...
export class TerrainAdaptationEngine {
  ...
  boxes(candidate, origin) {
    ...
    if (list.length) return list.map(b => ({
      ...
      maxX: Number(b.maxX ?? (b.x ?? origin.x) + (b.sizeX ?? b.width ?? 1) - 1), maxY: Number(b.maxY ?? (b.y ?? origin.y) + (b.sizeY ?? b.height ?? 1) - 1), maxZ: Number(b.maxZ ?? (b.z ?? origin.z) + (b.sizeZ ?? b.depth ?? 1) - 1)
    }));
    ...
    return [{ minX: origin.x - hx, minY: origin.y, minZ: origin.z - hz, maxX: origin.x + hx, maxY: origin.y + Number(s.y ?? 1) - 1, maxZ: origin.z + hz }];
  }

  union(boxes) { return boxes.reduce((a, b) => ({ minX: Math.min(a.minX, b.minX), minY: Math.min(a.minY, b.minY), minZ: Math.min(a.minZ, b.minZ), maxX: Math.max(a.maxX, b.maxX), maxY: Math.max(a.maxY, b.maxY), maxZ: Math.max(a.maxZ, b.maxZ) }), boxes[0]); }
  ...
  kernelAt(x, y, z) {
    ...
    const ix = x + BEARD_KERNEL_RADIUS, iy = y + BEARD_KERNEL_RADIUS, iz = z + BEARD_KERNEL_RADIUS;
    return BEARD_KERNEL[(ix * BEARD_KERNEL_SIZE + iy) * BEARD_KERNEL_SIZE + iz];
  }

  weightTableAt(x, y, z) {
    if (!this.isInKernelRange(x) || !this.isInKernelRange(y) || !this.isInKernelRange(z)) return 0;
    const ix = x + STRUCTURE_WEIGHT_RADIUS, iy = y + STRUCTURE_WEIGHT_RADIUS, iz = z + STRUCTURE_WEIGHT_RADIUS;
    return STRUCTURE_WEIGHT_TABLE[(ix * STRUCTURE_WEIGHT_SIZE + iy) * STRUCTURE_WEIGHT_SIZE + iz];
  }

  // Port of Beardifier.getBuryContribution.
  // Vanilla's buried-piece contribution is a compact spherical falloff:
  //   d = sqrt(x^2 + z^2)
  //   y = y + 0.5
  //   contribution = max(0, 1 - sqrt(d^2 + y^2) / 6)
  // The 0.5 vertical offset is important for the piece-to-terrain transition.
  getBuryContribution(x, y, z) {
    const vertical = y + 0.5;
    return Math.max(0, 1.0 - Math.sqrt(sq(x) + sq(vertical) + sq(z)) / 6.0);
  }

  // Port of Beardifier.getBeardContribution. Vanilla samples the precomputed
  // Gaussian kernel and attenuates it by the supplied structure height.
  getBeardContribution(x, y, z, height) {
    const kernel = this.kernelAt(x, y, z);
    if (kernel === 0) return 0;
    return kernel * Math.exp(-sq(height) / 16.0);
  }

  // Port of the integer-offset computeBeardContribution path.
  computeBeardContribution(x, y, z) {
    const horizontal = Math.sqrt(sq(x) + sq(z));
    if (horizontal >= BEARD_KERNEL_RADIUS) return 0;
    const kernel = this.kernelAt(x, y, z);
    return kernel * Math.max(0, 1.0 - horizontal / BEARD_KERNEL_RADIUS);
  }

  // Port of the floating-Y computeBeardContribution path used by density
  // sampling. This preserves the kernel lookup for horizontal integer offsets
  // while interpolating the vertical distance continuously.
  computeBeardContribution(x, y, z) {
    const horizontal = Math.sqrt(sq(x) + sq(z));
    if (horizontal >= BEARD_KERNEL_RADIUS) return 0;
    const lower = Math.floor(y), upper = lower + 1, t = y - lower;
    const a = this.kernelAt(x, lower, z), b = this.kernelAt(x, upper, z);
    return (a + (b - a) * t) * Math.max(0, 1.0 - horizontal / BEARD_KERNEL_RADIUS);
  }

  // Structure-piece distance to the nearest point of its bounding box.
  nearest(box, x, y, z) {
    return { x: x < box.minX ? box.minX - x : x > box.maxX ? x - box.maxX : 0, y: y < box.minY ? box.minY - y : y > box.maxY ? y - box.maxY : 0, z: z < box.minZ ? box.minZ - z : z > box.maxZ ? z - box.maxZ : 0 };
  }

  // Port of the Beardifier structure-weight path. Buried pieces use the
  // spherical bury contribution; rigid/beard pieces use the beard kernel.
  getStructureWeight(x, y, z, box, terrainAdjustment = "beard_thin") {
    const d = this.nearest(box, x, y, z);
    if (terrainAdjustment === "bury" || terrainAdjustment === "buried") return this.getBuryContribution(d.x, d.y, d.z);
    if (terrainAdjustment === "beard_box") return this.getBeardContribution(d.x, d.y, d.z, d.y);
    return this.getBeardContribution(d.x, d.y, d.z, d.y);
  }
  ...
  junctionWeight(x, y, z, junction) {
    ...
    return this.computeBeardContribution(Math.floor(dx), dy, Math.floor(dz));
  }

  calculateStructureWeight(x, y, z, boxes, junctions, mode) {
    let weight = 0;
    for (const box of boxes) weight += this.getStructureWeight(x, y, z, box, mode);
    for (const junction of junctions) weight += this.junctionWeight(x, y, z, junction);
  ...
  adapt(candidate, context = {}) {
    ...
    const radius = mode === "beard_box" || mode === "beard_thin" ? BEARD_KERNEL_RADIUS : 0;
    ...
    if (mode === "bury" || mode === "buried") {
      ...
      for (let x = bounds.minX - radius; x <= bounds.maxX + radius; x++) for (let z = bounds.minZ - radius; z <= bounds.maxZ + radius; z++) {
        ...
        for (let y = Math.max(-64, bounds.minY - depth); y < Math.min(surface, bounds.minY); y++) {
          let contribution = 0;
          for (const box of boxes) { const d = this.nearest(box, x, y, z); contribution = Math.max(contribution, this.getBuryContribution(d.x, d.y, d.z)); }
    ...
    for (let x = bounds.minX - radius; x <= bounds.maxX + radius; x++) for (let z = bounds.minZ - radius; z <= bounds.maxZ + radius; z++) {
      const surface = this.surfaceY(x, z);
      for (let y = Math.max(-64, targetY - BEARD_KERNEL_RADIUS); y <= Math.min(320, surface + 2); y++) {
        const density = this.calculateStructureWeight(x, y, z, boxes, junctions, mode);
        if (density <= 0) continue;
        result.densitySamples++;
        // The Beardifier is a density-function modifier. At the block API
        // boundary we apply the same contribution as a terrain-height blend.
        // Strong contributions excavate; weak contributions fill.
  ...
  flatten(candidate, context = {}) {
    ...
    for (let x = bounds.minX; x <= bounds.maxX; x++) for (let z = bounds.minZ; z <= bounds.maxZ; z++) {
      ...
      else if (s > targetY && ap) for (let y = targetY; y < s; y++) { const b = blockAt(this.dimension, { x, y, z }); if (b && !air(b) && !water(b)) try { this.dimension.setBlockPermutation({ x, y, z }, ap); changed++; } catch {} }
    }
