...
const CATEGORY_RULES = Object.freeze([
  ...
  { prefix: "endcity/", category: StructureCategory.SKY },
  { prefix: "mansion/", category: StructureCategory.SKY },
  { prefix: "bastion/", category: StructureCategory.SKY },
  { prefix: "coralcrust/", category: StructureCategory.WATER },
  { prefix: "fossils/", category: StructureCategory.UNDERGROUND },
  { prefix: "nether_fossils/", category: StructureCategory.UNDERGROUND }
]);
...
function isActualPackStructure(raw) {
  const n = normalize(raw);
  return n.length > 0 && !n.endsWith("/index") && !n.includes("/index/");
}
...
export class StructureRegistry {
  constructor(options = {}) {
    ...
    this.native = new Map();
    this.generated = options.generatedData ?? getGeneratedJigsawData();
    this.initialized = false;
    this.mappingValidation = [];
  }
  ...
  refresh() {
    ...
    this.native.clear();
    let ids = [];
    try { ids = world.structureManager?.getPackStructureIds?.() ?? []; }
    catch (e) { console.warn(`[Sky Archipelago] pack structure enumeration failed: ${e}`); }

    for (const raw of ids) {
      if (!isActualPackStructure(raw)) continue;
      const family = assetFamily(raw);
      if (!family) continue;
      const n = normalize(raw);
      const generatedPieceData = generatedPiece(raw);
      this.entries.set(n, {
        id: raw,
        normalized: n,
        ...
        family,
        role: assetRole(raw),
        kind: ASSET_ROLES[family]?.kind ?? "template",
        generated: generatedPieceData ?? null,
        generatedPath: generatedPieceData?.source ?? null
      });
    }

    for (const [id, category] of Object.entries(NATIVE_FAMILIES)) {
      this.native.set(normalize(id), {
        id,
    ...
    this.mappingValidation = validateAssetMapping([...this.entries.values()]);
  ...
  get(id) {
    this.ensure();
    const entry = this.entries.get(normalize(id));
  ...
  generatedJigsaw(id) { return generatedStructure(id); }
  generatedPiece(id) { return generatedPiece(id); }
  generatedPool(id) { return generatedPool(id); }
  ...
  select(seed, category, family = null, role = null) {
    const candidates = family ? (role ? this.byRole(family, role) : this.byFamily(family)) : this.byCategory(category);
  ...
  snapshot() {
    return {
      templates: [...this.ensure().values()].map(e => ({ id: e.id, category: e.category, family: e.family, role: e.role, generatedPath: e.generatedPath })),
      native: [...this.native.values()],
      generated: {
        pools: Object.keys(this.generated.template_pools ?? {}).length,
        structures: Object.keys(this.generated.structures ?? {}).length,
        processors: Object.keys(this.generated.processors ?? {}).length,
        structureSets: Object.keys(this.generated.structure_sets ?? {}).length,
        pieces: Object.keys(this.generated.pieces ?? {}).length
      },
      mapping: this.mappingValidation
    };
  }
}
