...
const DB="sky_archipelago:generated_v3", SEED="sky_archipelago:layout_seed_v3";
export class IslandGenerator {
 constructor(){this.noise=null;this.clusters=new IslandClusterSampler(null);this.factory=null;this.shape=null;this.density=null;this.generated=new Set();this.queue=[];this.queued=new Set();this.jobs=[];}
 load(){if(this.noise)return;let raw=world.getDynamicProperty(DB),saved=world.getDynamicProperty(SEED);if(typeof saved==="number")this.seed=BigInt(saved);else{this.seed=BigInt(Date.now());world.setDynamicProperty(SEED,Number(this.seed&2147483647n));}this.noise=new IslandNoise(this.seed);this.clusters=new IslandClusterSampler(this.noise);this.factory=new IslandDescriptorFactory(this.noise);this.shape=new IslandShapeSampler(this.noise);this.density=new IslandDensityEvaluator(this.noise,this.shape);try{if(typeof raw==="string"){const d=JSON.parse(raw);if(d.version===3&&Array.isArray(d.generated))this.generated=new Set(d.generated);}}catch{} }
 save(){world.setDynamicProperty(DB,JSON.stringify({version:3,generated:[...this.generated]}));}
