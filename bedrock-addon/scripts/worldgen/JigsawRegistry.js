...
import { vanillaFallbackInfo } from "./VanillaStartPoolFallbacks.js";
...
export class JigsawRegistry{
 ...
 candidates(poolId,targetName=null){const pool=this.pool(poolId);if(!pool)return[];const result=[];for(const wrapper of pool.elements??[]){const element=wrapper?.element??wrapper;if(!element||typeof element!=="object")continue;const type=element.element_type??element.elementType;if(type==="minecraft:empty_pool_element"||type==="minecraft:list_pool_element"){result.push({weight:normalizeWeight(wrapper?.weight),element,targetName});continue;}if((type==="minecraft:single_pool_element"||type==="minecraft:legacy_single_pool_element")&&element.location)result.push({weight:normalizeWeight(wrapper?.weight),element,targetName,piece:this.piece(element.location),resolvedPath:this.resolvedTemplate(element.location),processor:this.processor(element.processors??element.processor),projection:element.projection??"rigid"});}return result;}
 ...
 resolveStartPool(id){const structureId=generatedTemplateId(id),definition=this.structure(structureId)??{},explicit=definition.start_pool??definition.startPool;if(explicit&&this.pool(explicit))return{kind:"jigsaw",source:"generated",startPool:explicit,javaStartPools:[explicit],available:true};const fallback=vanillaFallbackInfo(structureId);if(fallback.kind==="jigsaw"){const available=fallback.javaStartPools.find(p=>Boolean(this.pool(p)));return{...fallback,startPool:available??null,available:Boolean(available),source:available?"vanilla-fallback":"missing"};}const root=fallback.javaRootPieces?.find(p=>Boolean(this.piece(p)||this.resolvedTemplate(p)));return{...fallback,rootPiece:root??null,available:Boolean(root)||Boolean(this.nativeJson("structures",structureId)),source:(root||this.nativeJson("structures",structureId))?"native-fallback":"native-adapter"};}
 ...
 validateStructure(id,maxDepth=20){const identifier=generatedTemplateId(id),s=this.structure(identifier);if(!s)return{valid:false,errors:[`Missing structure: ${identifier}`]};const resolved=this.resolveStartPool(identifier);if(resolved.kind==="native")return{valid:true,errors:[],identifier,definition:s,startPool:null,fallback:resolved};if(!resolved.startPool)return{valid:false,errors:[`${identifier}: missing start_pool (no generated Java/Bedrock-equivalent pool)`],identifier,definition:s,fallback:resolved};return{...this.validatePoolGraph(resolved.startPool,maxDepth),identifier,definition:s,startPool:resolved.startPool,fallback:resolved};}
