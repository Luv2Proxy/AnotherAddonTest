...
export class JigsawExpansionEngine {
  ...
  expand(startPieceId, startLocation, seed = "0", options = {}) {
    ...
    while (queue.length && placed.length < Math.min(this.maxPieces, Number(options.maxPieces ?? this.maxPieces))) {
      ...
      for (const connector of this.connectorResolver.connectors(piece)) {
        ...
        queue.push({ id: selected.id, location, depth: current.depth + 1, rotation, parent: current.id });
      }
    }
    return placed;
  }
