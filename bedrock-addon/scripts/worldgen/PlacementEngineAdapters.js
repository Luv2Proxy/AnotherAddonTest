...
const FAMILY_RULES = [
  ...
  ["ancient_city", StructureCategory.UNDERGROUND],
  ["trial_chambers", StructureCategory.UNDERGROUND],
  ["monument", StructureCategory.WATER],
  ["shipwreck", StructureCategory.WATER],
  ["underwater_ruin", StructureCategory.WATER],
  ["coral", StructureCategory.WATER],
  ["village/", StructureCategory.GROUND_VILLAGE],
  ["trail_ruins", StructureCategory.GROUND_VILLAGE],
  ["pillageroutpost", StructureCategory.SURFACE_SKY],
  ["igloo", StructureCategory.SMALL_SKY],
];

function idOf(candidate) {
  return String(candidate?.structure ?? candidate?.id ?? "").toLowerCase();
}
...
export function hasJigsawMetadata(registry, candidate) {
  ...
  const family = candidate?.family ?? id;
  try {
    if (registry?.hasGeneratedJigsaw?.(family)) return true;
    if (registry?.hasGeneratedJigsaw?.(id)) return true;
    if (registry?.generatedJigsaw?.(id)) return true;
  } catch {}
  return false;
}

export function classifyPlacement(registry, candidate) {
  const category = classifyStructure(candidate);
  const id = idOf(candidate);
...
function adapter(generator, names) {
  for (const name of names) {
    const fn = generator?.[name];
    if (typeof fn === "function") return fn.bind(generator);
  }
  return null;
}
...
export function createPlacementAdapters(generator) {
  return {
    sky: adapter(generator, ["placeSkyStructure", "placeSky", "placeTemplate"]),
    surface: adapter(generator, ["placeSurfaceStructure", "placeSurface", "placeTemplate"]),
    village: adapter(generator, ["placeGroundVillage", "placeVillage", "placeTemplate"]),
    water: adapter(generator, ["placeWaterStructure", "placeWater", "placeTemplate"]),
    underground: adapter(generator, ["placeUndergroundStructure", "placeUnderground", "placeTemplate"]),
    stronghold: adapter(generator, ["placeStronghold", "placeNative"]),
    jigsaw: adapter(generator, ["placeJigsawStructure", "placeJigsaw"]),
    template: adapter(generator, ["placeTemplate"]),
  };
}
...
export function adapterFor(adapters, classification) {
  ...
  if (adapters.sky) return adapters.sky;
