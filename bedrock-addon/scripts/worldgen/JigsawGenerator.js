...
export class JigsawGenerator {
  ...
  async projectPlan(plan,options={}){
    if(!plan?.pieces)return plan;
    const projection=options.projection??plan.projection??"rigid";
    if(projection==="rigid")return plan;
    const pieces=[];
    for(const piece of plan.pieces){const size=piece.size??piece.bounds?.size??{x:1,y:1,z:1};const origin=piece.origin??piece.location??{x:0,y:0,z:0};pieces.push({...piece,origin:await this.projectPiece(origin,size,projection,{...options,piece})});}
    return {...plan,pieces,projection};
  }
  ...
  placePool(pool,target,maxDepth,location,options={}){
    const poolId=String(pool).includes(":")?pool:`minecraft:${pool}`,depth=Math.max(1,Math.min(20,Number(maxDepth)||1));
    if(typeof this.manager?.placeJigsaw!=="function")return{placed:false,native:false,reason:"native_jigsaw_pool_api_unavailable",plan:this.planner.planPool(poolId,location,options.seed??this.layoutSeed,{...options,maxDepth:depth})};
    const bounds=this.manager.placeJigsaw(poolId,target??"",depth,this.dimension,location,{includeEntities:true,keepJigsaws:false,...options});
