...
export class TerrainAdaptationEngine{
 ...
 setBlock(position,blockId,states={}){const pos=p3(position),permutation=perm(blockId,states);if(!permutation||!this.dimension)return false;try{this.dimension.setBlockPermutation(pos,permutation);this.changes.push({position:pos,id:typeof blockId==="string"?blockId:null});return true;}catch{return false;}}
 surfaceY(x,z,hint=this.options.surfaceHintY??this.options.targetY??128){const minY=Number(this.options.minY??-64),maxY=Number(this.options.maxY??320),start=Math.max(minY,Math.min(maxY,Math.floor(Number(hint))));for(let y=start;y>=minY;y--){const b=blockAt(this.dimension,{x,y,z});if(b&&!air(b)&&!water(b))return y;}if(start<maxY)for(let y=start+1;y<=maxY;y++){const b=blockAt(this.dimension,{x,y,z});if(b&&!air(b)&&!water(b))return y;}return minY;}
 *surfaceYJob(x,z,hint=this.options.surfaceHintY??this.options.targetY??128){const minY=Number(this.options.minY??-64),maxY=Number(this.options.maxY??320),start=Math.max(minY,Math.min(maxY,Math.floor(Number(hint))));for(let y=start;y>=minY;y--){const b=blockAt(this.dimension,{x,y,z});yield;if(b&&!air(b)&&!water(b))return y;}if(start<maxY)for(let y=start+1;y<=maxY;y++){const b=blockAt(this.dimension,{x,y,z});yield;if(b&&!air(b)&&!water(b))return y;}return minY;}
 boxes(candidate,origin){const list=candidate?.pieceBounds??candidate?.boundingBoxes??candidate?.pieces?.map(p=>p.boundingBox??p.bounds).filter(Boolean)??[];if(list.length)return list.map(b=>({minX:Number(b.minX??b.x??origin.x),minY:Number(b.minY??b.y??origin.y),minZ:Number(b.minZ??b.z??origin.z),maxX:Number(b.maxX??(b.x??origin.x)+(b.sizeX??b.width??1)-1),maxY:Number(b.maxY??(b.y??origin.y)+(b.sizeY??b.height??1)-1),maxZ:Number(b.maxZ??(b.z??origin.z)+(b.sizeZ??b.depth??1)-1),groundLevelDelta:Number(b.groundLevelDelta??b.ground_level_delta??0),terrainAdjustment:b.terrainAdjustment??b.terrain_adjustment}));const s=candidate?.size??candidate?.transformedSize??{x:1,y:1,z:1};const hx=Math.max(1,Math.ceil(Number(candidate?.footprint?.x??candidate?.footprintRadius??s.x/2))),hz=Math.max(1,Math.ceil(Number(candidate?.footprint?.z??candidate?.footprintRadius??s.z/2)));return[{minX:origin.x-hx,minY:origin.y,minZ:origin.z-hz,maxX:origin.x+hx,maxY:origin.y+Number(s.y??1)-1,maxZ:origin.z+hz,groundLevelDelta:Number(candidate?.groundLevelDelta??0),terrainAdjustment:candidate?.terrain_adaptation}];}
 ...
 getBuryContribution(x,y,z){const vertical=Number(y)+.5,magnitude=Math.sqrt(sq(x)+sq(vertical)+sq(z));return magnitude>=6?0:1-magnitude/6;}
...
export function applyTerrainAdaptation(dimension,candidate,context={}){if(!dimension)return{mode:context.mode??"none",changed:0,skipped:"no_dimension"};const engine=new TerrainAdaptationEngine(dimension,context.options??{});return engine.scheduleAdaptation(candidate,context);}
